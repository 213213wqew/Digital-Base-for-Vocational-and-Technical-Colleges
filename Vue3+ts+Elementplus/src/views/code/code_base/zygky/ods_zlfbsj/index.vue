<template>
  <div class="patent-publication-management">
    <!-- 搜索区域 -->
    <el-card class="search-card" shadow="never">
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" class="search-form">
          <el-form-item label="专利编号" prop="fbzlbh">
            <el-input
              v-model="queryParams.fbzlbh"
              placeholder="请输入专利编号"
              clearable
              style="width: 220px;"
          ...
          <el-form-item label="专利名称" prop="fbzlmc">
            <el-input
              v-model="queryParams.fbzlmc"
              placeholder="请输入专利名称"
              clearable
              style="width: 220px;"
              @keyup.enter="handleQuery"
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">专利发布数据管理</div>
      ...
      <div class="table-body">
        ...
        >
          ...
          <el-table-column prop="fbzlbh" label="专利编号" min-width="150" show-overflow-tooltip />
          <el-table-column prop="fbzlmc" label="专利名称" min-width="200" show-overflow-tooltip />
          <el-table-column prop="sqggrq" label="授权公告日期" min-width="120" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px" :close-on-click-modal="false" class="data-form-dialog" destroy-on-close @close="handleFormClose">
      <el-form v-if="dialogMode !== 'view'" ref="formRef" :model="form" :rules="formRules" label-width="140px">
        <el-row :gutter="20">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="发布专利编号" prop="fbzlbh">
              <el-input v-model="form.fbzlbh" placeholder="请输入发布专利编号" maxlength="50" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="授权公告日期" prop="sqggrq">
              <el-date-picker v-model="form.sqggrq" type="date" placeholder="请选择授权公告日期" format="YYYYMMDD" value-format="YYYYMMDD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="发布专利名称" prop="fbzlmc">
              <el-input v-model="form.fbzlmc" placeholder="请输入发布专利名称" maxlength="200" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
          ...
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="发布专利简介" prop="fbzljj">
              <el-input v-model="form.fbzljj" type="textarea" :rows="4" placeholder="请输入发布专利简介" maxlength="2000" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
      ...
      <div v-else class="detail-view">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="发布专利编号">{{ form.fbzlbh }}</el-descriptions-item>
          <el-descriptions-item label="发布专利名称" :span="2">{{ form.fbzlmc }}</el-descriptions-item>
          <el-descriptions-item label="授权公告日期">{{ form.sqggrq }}</el-descriptions-item>
          ...
          <el-descriptions-item label="发布专利简介" :span="2">{{ form.fbzljj || '无' }}</el-descriptions-item>
...
import {
  ...
  updateOdsZlfbsj,
  deleteOdsZlfbsj,
  batchDeleteOdsZlfbsj,
  exportOdsZlfbsj,
  getSystemSchoolCode, // 假设API文件中会添加
  type OdsZlfbsjInfo,
  type OdsZlfbsjQuery,
  type OdsZlfbsjForm,
} from "@/api-code/code/code_base/zygky/ods_zlfbsj";

const loading = ref(false);
const dataList = ref<OdsZlfbsjInfo[]>([]);
const total = ref(0);
...
const dialogTitle = computed(() => {
  switch (dialogMode.value) {
    case "add": return "新增专利发布数据";
    case "edit": return "编辑专利发布数据";
    case "view": return "查看专利发布数据";
    default: return "专利发布数据";
  }
});

const queryParams = reactive<OdsZlfbsjQuery>({
  zjsjwyxbs: "",
  xxdm: "",
  fbzlbh: "",
  fbzlmc: "",
...
const form = reactive<OdsZlfbsjForm>({
  id: undefined,
  zjsjwyxbs: "",
  xxdm: "",
  fbzlbh: "",
...
const formRules = reactive({
  ...
  fbzlbh: [{ required: true, message: "发布专利编号是必填项", trigger: "blur" }, { max: 50, message: "长度不超过50个字符", trigger: "blur" }],
  fbzlmc: [{ required: true, message: "发布专利名称是必填项", trigger: "blur" }, { max: 200, message: "长度不超过200个字符", trigger: "blur" }],
  sqggrq: [{ required: true, message: "授权公告日期是必填项", trigger: "change" }],
...
const getList = async () => {
  loading.value = true;
  try {
    const response = await getOdsZlfbsjPage(queryParams);
...
const handleEdit = async (row: OdsZlfbsjInfo) => {
  resetForm();
  const response = await getOdsZlfbsjById(row.id);
  Object.assign(form, response.data.data);
  dialogMode.value = "edit";
  dialogVisible.value = true;
};

const handleView = async (row: OdsZlfbsjInfo) => {
  resetForm();
  const response = await getOdsZlfbsjById(row.id);
  Object.assign(form, response.data.data);
...
const handleDelete = async (row: OdsZlfbsjInfo) => {
  await ElMessageBox.confirm("确定要删除这条数据吗？", "提示", { type: "warning" });
  await deleteOdsZlfbsj(row.id);
...
const handleBatchDelete = async () => {
  ...
  await batchDeleteOdsZlfbsj(selectedIds.value);
  ElMessage.success("删除成功");
  getList();
};

const handleExport = async () => {
  const response = await exportOdsZlfbsj(queryParams);
  const blob = new Blob([response.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `专利发布数据_${new Date().getTime()}.xlsx`;
  link.click();
  URL.revokeObjectURL(link.href);
};

const handleStatusChange = async (row: OdsZlfbsjInfo) => {
  try {
    const response = await updateOdsZlfbsj({ id: row.id, enableStatus: row.enableStatus });
...
const handleFormSubmit = async () => {
  ...
  try {
    if (dialogMode.value === "add") {
      await addOdsZlfbsj(form);
      ElMessage.success("新增成功");
    } else {
      await updateOdsZlfbsj(form);
...
const handleSelectionChange = (selection: OdsZlfbsjInfo[]) => {
