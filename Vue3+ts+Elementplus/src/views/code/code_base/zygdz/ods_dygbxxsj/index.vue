  ...
  <div class="party-cadre-study-management">
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">党员干部学习数据管理</div>
    ...
    >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="党组织名称" prop="dzzmc">
              ...
              <el-tooltip content="数据来源于党组织情况基础数据子类" placement="top">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="学习培训途径载体码" prop="dygbxxpxzytjhztm">
              <el-select
                v-model="form.dygbxxpxzytjhztm"
                placeholder="请选择学习培训途径载体码"
                style="width: 100%"
                clearable
                multiple
                collapse-tags
              ...
              >
                <el-option
                  v-for="item in dictData.dygbxxpxzytjhztmOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="学习培训内容码" prop="dygbxxpxnrm">
              <el-select
                v-model="form.dygbxxpxnrm"
                placeholder="请选择学习培训内容码"
                style="width: 100%"
                clearable
                multiple
                collapse-tags
              ...
              >
                <el-option
                  v-for="item in dictData.dygbxxpxnrmOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
      ...
      <!-- 查看模式 -->
      <div v-else class="party-cadre-study-detail">
        <el-descriptions
        ...
        >
          ...
          <el-descriptions-item label="学习培训途径载体">
            <div class="detail-value">{{ form.dygbxxpxzytjhztmText || form.dygbxxpxzytjhztm || "-" }}</div>
          </el-descriptions-item>
          <el-descriptions-item label="学习培训内容">
            <div class="detail-value">{{ form.dygbxxpxnrmText || form.dygbxxpxnrm || "-" }}</div>
          </el-descriptions-item>
...
import {
  ...
  updateOdsDygbxxsj,
  deleteOdsDygbxxsj,
  batchDeleteOdsDygbxxsj,
  exportOdsDygbxxsj,
  getDictOptions,
  searchOrgInfo,
  getSystemSchoolCode,
  type OdsDygbxxsjInfo,
  type OdsDygbxxsjQuery,
  type OdsDygbxxsjForm,
  type OrgOption,
} from "@/api-code/code/code_base/zygdz/ods_dygbxxsj";

// 响应式数据
const loading = ref(false);
const dataList = ref<OdsDygbxxsjInfo[]>([]);
const total = ref(0);
...
// 搜索条件
const queryParams = reactive<OdsDygbxxsjQuery>({
  zjsjwyxbs: "",
  xxdm: "",
  dzzmc: "",
  dzzbh: "",
...
// 表单相关
const form = reactive<OdsDygbxxsjForm>({
  zjsjwyxbs: "",
  xxdm: "",
  dzzmc: "",
  dzzbh: "",
...
const formRules = reactive({
  ...
  dygbxxpxzytjhztm: [
    { required: true, message: "请选择学习培训途径载体码", trigger: "change" }
  ],
  dygbxxpxnrm: [
    { required: true, message: "请选择学习培训内容码", trigger: "change" }
  ],
  hdksrq: [
  ...
  cyxss: [
    ...
    { type: 'number', min: 0, max: 99999, message: "参与学生数必须在0-99999之间", trigger: "blur" }
  ],
  cyjss: [
    { required: true, message: "请输入参与教师数", trigger: "blur" },
    { type: 'number', min: 0, max: 99999, message: "参与教师数必须在0-99999之间", trigger: "blur" }
  ],
  sjcjsj: [
...
const dictData = reactive({
  dygbxxpxzytjhztmOptions: [], // 党建活动主要途径和载体代码选项
  dygbxxpxnrmOptions: [], // 党员干部学习培训内容代码选项
});
...
const loadDictData = async () => {
  try {
    // 并行获取所有字典数据
    const [dygbxxpxzytjhztmRes, dygbxxpxnrmRes] = await Promise.all([
      getDictOptions('DJHDZYTJHZTDM'), // 党建活动主要途径和载体代码
      getDictOptions('DYGBXXPXNRDM'), // 党员干部学习培训内容代码
    ]);
    
    // 处理返回的数据
    if (dygbxxpxzytjhztmRes.data.code === 200) {
      dictData.dygbxxpxzytjhztmOptions = dygbxxpxzytjhztmRes.data.data || [];
    }
    if (dygbxxpxnrmRes.data.code === 200) {
      dictData.dygbxxpxnrmOptions = dygbxxpxnrmRes.data.data || [];
    }
  } catch (error) {
...
const handleOrgSearch = async (query: string) => {
  const response: any = await searchOrgInfo(query);
...
const handleOrgChange = (value: string) => {
  const selectedOrg = orgOptions.value.find(org => org.value === value);
  if (selectedOrg) {
...
const getList = async () => {
  loading.value = true;
  try {
    const response: any = await getOdsDygbxxsjPage(queryParams);
    if (response.data.code === 200) {
    ...
    console.error("查询党员干部学习数据失败:", error);
...
// 编辑
const handleEdit = async (row: OdsDygbxxsjInfo) => {
  try {
    const response: any = await getOdsDygbxxsjById(row.id);
    if (response.data.code === 200) {
      Object.assign(form, response.data.data);
      // 处理多选字段
      if (form.dygbxxpxzytjhztm && typeof form.dygbxxpxzytjhztm === 'string') {
        form.dygbxxpxzytjhztm = form.dygbxxpxzytjhztm.split(',');
      }
      if (form.dygbxxpxnrm && typeof form.dygbxxpxnrm === 'string') {
        form.dygbxxpxnrm = form.dygbxxpxnrm.split(',');
      }
    ...
    console.error("获取党员干部学习数据详情失败:", error);
    ElMessage.error("获取详情失败");
  }
};

// 查看详情
const handleView = async (row: OdsDygbxxsjInfo) => {
  try {
    const response: any = await getOdsDygbxxsjById(row.id);
    if (response.data.code === 200) {
      Object.assign(form, response.data.data);
...
// 删除
const handleDelete = async (row: OdsDygbxxsjInfo) => {
  try {
    await ElMessageBox.confirm("确定要删除这条党员干部学习数据吗？", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });

    const response: any = await deleteOdsDygbxxsj(row.id);
    if (response.data.code === 200) {
    ...
    if (error !== "cancel") {
      console.error("删除党员干部学习数据失败:", error);
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 条党员干部学习数据吗？`,
      "提示",
      {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
    ...
    const response: any = await batchDeleteOdsDygbxxsj(selectedIds.value);
    if (response.data.code === 200) {
    ...
    if (error !== "cancel") {
      console.error("批量删除党员干部学习数据失败:", error);
...
const handleExport = async () => {
  try {
    const response = await exportOdsDygbxxsj(queryParams);
    ...
    link.href = url;
    link.download = `党员干部学习数据_${new Date().getTime()}.xlsx`;
    ...
    console.error("导出党员干部学习数据失败:", error);
...
const handleFormSubmit = async () => {
  ...
  try {
    ...
    // 处理多选字段
    const formData = { ...form };
    if (Array.isArray(formData.dygbxxpxzytjhztm)) {
      formData.dygbxxpxzytjhztm = formData.dygbxxpxzytjhztm.join(',');
    }
    if (Array.isArray(formData.dygbxxpxnrm)) {
      formData.dygbxxpxnrm = formData.dygbxxpxnrm.join(',');
    }

    let response: any;
    if (dialogMode.value === "add") {
      response = await addOdsDygbxxsj(formData);
    } else {
      response = await updateOdsDygbxxsj(formData);
    }
...
// 表格选择变化
const handleSelectionChange = (selection: OdsDygbxxsjInfo[]) => {
  selectedIds.value = selection.map((item) => item.id);
};

// 行双击
const handleRowDblClick = (row: OdsDygbxxsjInfo) => {
  handleView(row);
};
