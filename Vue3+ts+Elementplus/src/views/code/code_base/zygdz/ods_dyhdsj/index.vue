...
import {
  ...
  type OdsDyhdsjForm,
  type DictOption,
} from "@/api-code/code/code_base/zygdz/ods_dyhdsj";
...
onMounted(async () => {
  await Promise.all([
    getSystemSchoolCode().then((response: any) => {
      systemSchoolCode.value = response.data.data;
    }),
    getDictOptions("DYHDXSDM").then((response: any) => {
...
const getList = async () => {
  try {
    loading.value = true;
    const response = await getOdsDyhdsjPage(queryParams.value) as any;
...
const handleEdit = async (row: OdsDyhdsjInfo) => {
  try {
    const response = await getOdsDyhdsjById(row.id) as any;
    const result = response.data;
    form.value = {
      ...result.data,
...
const handleDelete = (row: OdsDyhdsjInfo) => {
  ElMessageBox.confirm("确定要删除该条数据吗？", "提示", {
    ...
    .then(async () => {
      try {
        const response = await deleteOdsDyhdsj(row.id) as any;
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条数据吗？`, "提示", {
    ...
    .then(async () => {
      try {
        const response = await batchDeleteOdsDyhdsj(selectedIds.value) as any;
        if (response.data.code === 200) {
    ...
    if (valid) {
      try {
        ...
        const response = await (dialogMode.value === "add"
          ...
          : updateOdsDyhdsj(submitForm)) as any;
...
const handleOrgSearch = async (query: string) => {
  if (query) {
    try {
      ...
      const response = await searchOrgInfo(query) as any;
      const result = response.data;
