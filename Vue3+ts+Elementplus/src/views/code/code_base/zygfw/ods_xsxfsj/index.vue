<template>
  <div class="student-consumption-management">
    <!-- 搜索区域 -->
    <el-card class="search-card" shadow="never">
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" class="search-form">
          <el-form-item label="消费类别" prop="xflbm">
            <el-select
              v-model="queryParams.xflbm"
              placeholder="请选择消费类别"
              clearable
              style="width: 220px;"
            >
              <el-option
                v-for="item in dictData.xflbmOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item label="消费日期" prop="xfrq">
            <el-date-picker
              v-model="queryParams.xfrq"
              type="date"
              placeholder="请选择消费日期"
              format="YYYYMMDD"
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">学生消费数据编码管理</div>
      ...
      <div class="table-body">
        ...
        >
          ...
          <el-table-column prop="xfze" label="消费总额" min-width="120" />
          <el-table-column prop="xfrc" label="消费人次" min-width="110" align="center" />
          <el-table-column prop="xflbmText" label="消费类别" min-width="120" />
          <el-table-column prop="xfrq" label="消费日期" min-width="120" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px" :close-on-click-modal="false" class="data-form-dialog" destroy-on-close @close="handleFormClose">
      <el-form v-if="dialogMode !== 'view'" ref="formRef" :model="form" :rules="formRules" label-width="140px">
        <el-row :gutter="20">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="消费总额" prop="xfze">
              <el-input-number v-model="form.xfze" :precision="2" :min="0" placeholder="请输入消费总额" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="消费人次" prop="xfrc">
              <el-input-number v-model="form.xfrc" :min="0" placeholder="请输入消费人次" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="消费类别" prop="xflbm">
              <el-select v-model="form.xflbm" placeholder="请选择消费类别" style="width: 100%">
                <el-option
                  v-for="item in dictData.xflbmOptions"
                  :key="item.value"
                  :label="item.label"
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="消费日期" prop="xfrq">
              <el-date-picker v-model="form.xfrq" type="date" placeholder="请选择消费日期" format="YYYYMMDD" value-format="YYYYMMDD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
      ...
      <div v-else class="detail-view">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="消费总额">{{ form.xfze }}</el-descriptions-item>
          <el-descriptions-item label="消费人次">{{ form.xfrc }}</el-descriptions-item>
          <el-descriptions-item label="消费类别">{{ form.xflbmText }}</el-descriptions-item>
          <el-descriptions-item label="消费日期">{{ form.xfrq }}</el-descriptions-item>
...
import {
  ...
  type OdsXsxfsjQuery,
  type OdsXsxfsjForm,
} from "@/api-code/code/code_base/zygfw/ods_xsxfsj";

const loading = ref(false);
const dataList = ref<OdsXsxfsjInfo[]>([]);
const total = ref(0);
...
const dialogTitle = computed(() => {
  switch (dialogMode.value) {
    case "add": return "新增学生消费数据";
    case "edit": return "编辑学生消费数据";
    case "view": return "查看学生消费数据";
    default: return "学生消费数据";
  }
});

const queryParams = reactive<OdsXsxfsjQuery>({
  xflbm: "",
  xfrq: "",
  pageNum: 1,
  pageSize: 20,
...
const form = reactive<OdsXsxfsjForm>({
  id: undefined,
  zjsjwyxbs: "",
  xxdm: "",
  xfze: 0,
...
const formRules = reactive<FormRules>({
  ...
  xfze: [{ required: true, message: "消费总额是必填项", trigger: "blur" }],
  xfrc: [{ required: true, message: "消费人次是必填项", trigger: "blur" }],
  xflbm: [{ required: true, message: "消费类别是必填项", trigger: "change" }],
  xfrq: [{ required: true, message: "消费日期是必填项", trigger: "change" }],
...
const loadDictData = async () => {
  const response = await getDictOptions('XFLBDM');
  if (response.data.code === 200) {
    dictData.xflbmOptions = response.data.data;
  }
};
...
const getList = async () => {
  loading.value = true;
  try {
    const response = await getOdsXsxfsjPage(queryParams);
    if (response.data.code === 200) {
      dataList.value = response.data.data.records.map((item: OdsXsxfsjInfo) => ({
        ...item,
        xflbmText: dictData.xflbmOptions.find(d => d.value === item.xflbm)?.label || item.xflbm,
      }));
...
const handleEdit = async (row: OdsXsxfsjInfo) => {
  resetForm();
  const response = await getOdsXsxfsjById(row.id);
  Object.assign(form, response.data.data);
  dialogMode.value = "edit";
  dialogVisible.value = true;
};

const handleView = async (row: OdsXsxfsjInfo) => {
  resetForm();
  const response = await getOdsXsxfsjById(row.id);
  const data = response.data.data;
  data.xflbmText = dictData.xflbmOptions.find(d => d.value === data.xflbm)?.label || data.xflbm;
  Object.assign(form, data);
  dialogMode.value = "view";
  dialogVisible.value = true;
};

const handleDelete = async (row: OdsXsxfsjInfo) => {
  await ElMessageBox.confirm("确定要删除这条数据吗？", "提示", { type: "warning" });
  await deleteOdsXsxfsj(row.id);
...
const handleBatchDelete = async () => {
  ...
  await batchDeleteOdsXsxfsj(selectedIds.value);
  ElMessage.success("删除成功");
  getList();
};

const handleExport = async () => {
  const response = await exportOdsXsxfsj(queryParams);
  const blob = new Blob([response.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `学生消费数据_${new Date().getTime()}.xlsx`;
  link.click();
  URL.revokeObjectURL(link.href);
};

const handleStatusChange = async (row: OdsXsxfsjInfo) => {
  try {
    const response = await updateOdsXsxfsj({ id: row.id, enableStatus: row.enableStatus });
    if (response.data.code === 200) {
...
const handleFormSubmit = async () => {
  ...
  try {
    if (dialogMode.value === "add") {
      await addOdsXsxfsj(form);
      ElMessage.success("新增成功");
    } else {
      await updateOdsXsxfsj(form);
...
const handleSelectionChange = (selection: OdsXsxfsjInfo[]) => {
...
<style scoped>
.student-consumption-management {
  padding: 20px;
}
.search-card {
  margin-bottom: 8px;
