<template>
  <div class="paper-publication-management">
    <!-- 搜索区域 -->
    <el-card class="search-card" shadow="never">
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" class="search-form">
          ...
          <el-form-item label="论文名称" prop="lwzwmc">
            <el-input
              v-model="queryParams.lwzwmc"
              placeholder="请输入论文名称"
              clearable
              style="width: 220px;"
              @keyup.enter="handleQuery"
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">论文发表数据管理</div>
      ...
      <div class="table-body">
        ...
        >
          ...
          <el-table-column prop="lwbh" label="论文编号" min-width="150" show-overflow-tooltip />
          <el-table-column prop="lwzwmc" label="论文中文名称" min-width="200" show-overflow-tooltip />
          <el-table-column prop="fbrq" label="发表日期" min-width="120" />
          <el-table-column prop="fbkwmc" label="发表刊物名称" min-width="180" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px" :close-on-click-modal="false" class="data-form-dialog" destroy-on-close @close="handleFormClose">
      <el-form v-if="dialogMode !== 'view'" ref="formRef" :model="form" :rules="formRules" label-width="140px">
        <el-row :gutter="20">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="论文编号" prop="lwbh">
              <el-input v-model="form.lwbh" placeholder="请输入论文编号" maxlength="100" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="发表日期" prop="fbrq">
              <el-date-picker v-model="form.fbrq" type="date" placeholder="请选择发表日期" format="YYYYMMDD" value-format="YYYYMMDD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="论文中文名称" prop="lwzwmc">
              <el-input v-model="form.lwzwmc" placeholder="请输入论文中文名称" maxlength="500" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="发表刊物名称" prop="fbkwmc">
              <el-input v-model="form.fbkwmc" placeholder="请输入发表刊物名称" maxlength="180" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="第一完成人工号" prop="dywcrgh">
              <el-input v-model="form.dywcrgh" placeholder="请输入第一完成人工号" maxlength="150" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
          ...
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="论文简介" prop="lwjj">
              <el-input v-model="form.lwjj" type="textarea" :rows="4" placeholder="请输入论文简介" maxlength="2000" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
      ...
      <div v-else class="detail-view">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="论文编号">{{ form.lwbh }}</el-descriptions-item>
          <el-descriptions-item label="发表日期">{{ form.fbrq }}</el-descriptions-item>
          <el-descriptions-item label="论文中文名称" :span="2">{{ form.lwzwmc }}</el-descriptions-item>
          <el-descriptions-item label="发表刊物名称" :span="2">{{ form.fbkwmc }}</el-descriptions-item>
          <el-descriptions-item label="第一完成人工号">{{ form.dywcrgh || "无" }}</el-descriptions-item>
          <el-descriptions-item label="数据采集时间">{{ form.sjcjsj }}</el-descriptions-item>
          <el-descriptions-item label="论文简介" :span="2">{{ form.lwjj || "无" }}</el-descriptions-item>
...
import {
  ...
  updateOdsLwfbsj,
  deleteOdsLwfbsj,
  batchDeleteOdsLwfbsj,
  exportOdsLwfbsj,
  getSystemSchoolCode,
  type OdsLwfbsjInfo,
  type OdsLwfbsjQuery,
  type OdsLwfbsjForm,
} from "@/api-data/data/data_base/zygky/ods_lwfbsj";

const loading = ref(false);
const dataList = ref<OdsLwfbsjInfo[]>([]);
const total = ref(0);
...
const dialogTitle = computed(() => {
  switch (dialogMode.value) {
    case "add": return "新增论文发表数据";
    case "edit": return "编辑论文发表数据";
    case "view": return "查看论文发表数据";
    default: return "论文发表数据";
  }
});

const queryParams = reactive<OdsLwfbsjQuery>({
  zjsjwyxbs: "",
  lwzwmc: "",
  dywcrgh: "",
  pageNum: 1,
...
const form = reactive<OdsLwfbsjForm>({
  zjsjwyxbs: "",
  xxdm: "",
  lwbh: "",
  lwzwmc: "",
...
const formRules = reactive({
  ...
  lwbh: [{ required: true, message: "请输入论文编号", trigger: "blur" }],
  lwzwmc: [{ required: true, message: "请输入论文中文名称", trigger: "blur" }],
  fbrq: [{ required: true, message: "请输入发表日期", trigger: "blur" }],
  fbkwmc: [{ required: true, message: "请输入发表刊物名称", trigger: "blur" }],
  dywcrgh: [{ required: true, message: "请输入第一完成人工号", trigger: "blur" }],
...
const getList = async () => {
  loading.value = true;
  try {
    const response = await getOdsLwfbsjPage(queryParams);
...
const handleEdit = async (row: OdsLwfbsjInfo) => {
  resetForm();
  const response = await getOdsLwfbsjById(row.id);
  Object.assign(form, response.data.data);
  dialogMode.value = "edit";
  dialogVisible.value = true;
};

const handleView = async (row: OdsLwfbsjInfo) => {
  resetForm();
  const response = await getOdsLwfbsjById(row.id);
  Object.assign(form, response.data.data);
...
const handleRowDblClick = (row: OdsLwfbsjInfo) => {
  handleView(row);
};

const handleDelete = async (row: OdsLwfbsjInfo) => {
  await ElMessageBox.confirm("确定要删除这条数据吗？", "提示", { type: "warning" });
  await deleteOdsLwfbsj(row.id);
...
const handleBatchDelete = async () => {
  ...
  await batchDeleteOdsLwfbsj(selectedIds.value);
  ElMessage.success("删除成功");
  getList();
};

const handleExport = async () => {
  const response = await exportOdsLwfbsj(queryParams);
  const blob = new Blob([response.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `论文发表数据_${new Date().getTime()}.xlsx`;
  link.click();
  URL.revokeObjectURL(link.href);
};

const handleStatusChange = async (row: OdsLwfbsjInfo) => {
  try {
    const response = await updateOdsLwfbsj({ id: row.id, enableStatus: row.enableStatus });
...
const handleFormSubmit = async () => {
  ...
  try {
    if (dialogMode.value === "add") {
      await addOdsLwfbsj(form);
      ElMessage.success("新增成功");
    } else {
      await updateOdsLwfbsj(form);
...
const handleSelectionChange = (selection: OdsLwfbsjInfo[]) => {
