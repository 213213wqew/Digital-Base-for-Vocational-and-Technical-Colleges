<template>
  <div class="system-building-management">
      ...
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true">
          <el-form-item label="所属部门名称" prop="bmmc">
            <el-input v-model="queryParams.bmmc" placeholder="请输入所属部门名称" clearable />
          </el-form-item>
          <el-form-item label="信息化系统编号" prop="xxhxtbh">
            <el-input v-model="queryParams.xxhxtbh" placeholder="请输入信息化系统编号" clearable />
          </el-form-item>
          <el-form-item label="采集时间范围" prop="sjcjsjRange">
            <el-date-picker v-model="sjcjsjRange" type="datetimerange" range-separator="至" start-placeholder="开始时间"
              end-placeholder="结束时间" format="YYYY-MM-DD HH:mm:ss" value-format="YYYYMMDD HHmmss"
    ...
    <el-card class="table-card" shadow="never">
      ...
      <div class="table-body">
        <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange"
          @sort-change="handleSortChange" border style="width: 100%" :height="'calc(80vh - 140px)'"
          highlight-current-row stripe size="default" :row-style="{ height: '32px' }"
          :cell-style="{ padding: '12px 6px' }">
          <el-table-column type="selection" width="45" fixed="left" />
          <el-table-column type="index" label="序号" width="50" fixed="left" />

          <el-table-column prop="zjsjwyxbs" show-overflow-tooltip label="主键数据唯一性标识" min-width="180" sortable="custom" />
          <el-table-column prop="xxdm" show-overflow-tooltip label="学校代码" min-width="120" sortable="custom" />
          <el-table-column prop="bmmc" show-overflow-tooltip label="所属部门名称" min-width="150" sortable="custom" />
          <el-table-column prop="xxhxtbh" show-overflow-tooltip label="信息化系统编号" min-width="140" sortable="custom" />
          <el-table-column prop="xxhxtflm" show-overflow-tooltip label="信息化系统分类码" min-width="140" sortable="custom">
            <template #default="{ row }">
              {{ getSystemTypeText(row.xxhxtflm) }}
            </template>
          </el-table-column>
          <el-table-column prop="xxhxtjc" show-overflow-tooltip label="信息化系统简称" min-width="140" sortable="custom" />
          <el-table-column prop="xxhxtqc" show-overflow-tooltip label="信息化系统全称" min-width="180" sortable="custom" />
          <el-table-column prop="xxhxtfwdz" label="信息化系统访问地址" min-width="200" sortable="custom">
            <template #default="scope">
              <el-link v-if="scope.row.xxhxtfwdz" :href="scope.row.xxhxtfwdz" target="_blank" type="primary">
                {{ scope.row.xxhxtfwdz }}
              </el-link>
              <span v-else>-</span>
            </template>
          </el-table-column>
          <el-table-column prop="sftsxxhxt" label="是否特色信息化系统" min-width="150" sortable="custom">
            <template #default="{ row }">
              <el-tag :type="row.sftsxxhxt === '1' ? 'success' : 'info'" size="small">
                {{ getIsSpecialSystemText(row.sftsxxhxt) }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="cjjcjrfs" label="场景击穿接入方式" min-width="150" sortable="custom">
            <template #default="{ row }">
              {{ getAccessModeText(row.cjjcjrfs) }}
            </template>
          </el-table-column>
          <el-table-column prop="xxhxtms" show-overflow-tooltip label="信息化系统描述" min-width="180" sortable="custom" />
          <el-table-column prop="xxhxtjslxm" label="信息化系统建设类型码" min-width="160" sortable="custom">
            <template #default="{ row }">
              {{ getBuildingTypeText(row.xxhxtjslxm) }}
            </template>
          </el-table-column>
          <el-table-column prop="sjcjsj" show-overflow-tooltip label="数据采集时间" min-width="160" sortable="custom">
            <template #default="{ row }">
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px" :close-on-click-modal="false" append-to-body
      class="system-building-dialog" destroy-on-close>
      <!-- 表单内容 -->
      <div class="system-building-form">
        <el-form ref="formRef" :model="form" :rules="formRules" label-width="140px" class="form-container dialog-form"
          inline-message>
          <!-- 基础信息 -->
          <el-row :gutter="10">
          ...
          <el-row :gutter="10">
            <el-col :span="12">
              <el-form-item label="所属部门名称" prop="bmmc">
                <el-input v-model="form.bmmc" placeholder="请输入所属部门名称" maxlength="200" show-word-limit />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="信息化系统编号" prop="xxhxtbh">
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="信息化系统建设类型" prop="xxhxtjslxm">
                <el-select v-model="form.xxhxtjslxm" placeholder="请选择信息化系统建设类型" style="width: 100%" clearable>
                  <el-option v-for="item in buildingTypeOptions" :key="item.value" :label="item.label"
                    :value="item.value" />
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="10">
            <el-col :span="12">
              <el-form-item label="信息化系统分类" prop="xxhxtflm">
                <el-select v-model="form.xxhxtflm" placeholder="请选择信息化系统分类" style="width: 100%" clearable>
            ...
            <el-col :span="12">
              <el-form-item label="是否特色信息化系统" prop="sftsxxhxt">
                <el-select v-model="form.sftsxxhxt" placeholder="请选择是否特色信息化系统" style="width: 100%" clearable
                  @change="handleSpecialSystemChange">
                  <el-option v-for="item in isSpecialSystemOptions" :key="item.value" :label="item.label"
                    :value="item.value" />
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="10">
            <el-col :span="12">
              <el-form-item label="场景击穿接入方式" prop="cjjcjrfs">
                <el-select v-model="form.cjjcjrfs" placeholder="请选择场景击穿接入方式" style="width: 100%" clearable>
                  <el-option v-for="item in accessModeOptions" :key="item.value" :label="item.label"
                    :value="item.value" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="数据采集时间" prop="sjcjsj">
                <el-date-picker v-model="form.sjcjsj" type="datetime" placeholder="请选择数据采集时间" style="width: 100%"
                  value-format="YYYYMMDD HHmmss" format="YYYY-MM-DD HH:mm:ss" />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="10">
            <el-col :span="24">
              <el-form-item label="信息化系统全称" prop="xxhxtqc">
                <el-input v-model="form.xxhxtqc" placeholder="请输入信息化系统全称" maxlength="200" show-word-limit />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="10">
            <el-col :span="24">
              <el-form-item label="信息化系统访问地址" prop="xxhxtfwdz">
                <el-input v-model="form.xxhxtfwdz" placeholder="请输入信息化系统访问地址（如：https://example.com）" maxlength="1500"
                  show-word-limit />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="10">
            <el-col :span="24">
              <el-form-item label="信息化系统描述" prop="xxhxtms">
                <el-input v-model="form.xxhxtms" type="textarea" :rows="3" placeholder="请输入信息化系统描述（150字以内）"
          ...
          <el-row :gutter="10">
            <el-col :span="12">
              <el-form-item label="上报状态" prop="reportStatus">
                <el-select v-model="form.reportStatus" placeholder="请选择上报状态" style="width: 100%" clearable>
                  <el-option label="未上报" value="0" />
                  <el-option label="已上报" value="1" />
                  <el-option label="上报失败" value="2" />
                </el-select>
            ...
            <el-col :span="12">
              <el-form-item label="启用状态" prop="enableStatus">
                <el-select v-model="form.enableStatus" placeholder="请选择启用状态" style="width: 100%" clearable>
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="detailVisible" title="信息化系统建设数据详情" width="900px" append-to-body
      class="detail-dialog draggable-dialog" destroy-on-close>
      <!-- 详情内容 -->
      <div class="system-building-detail">
        <el-descriptions :column="2" border class="detail-descriptions">
          ...
          <el-descriptions-item label="是否特色信息化系统">
            <el-tag :type="detailData.sftsxxhxt === '1' ? 'success' : 'info'" size="small">
          ...
          <el-descriptions-item label="信息化系统访问地址" :span="2">
            <el-link v-if="detailData.xxhxtfwdz" :href="detailData.xxhxtfwdz" target="_blank" type="primary"
              class="detail-link">
          ...
          <el-descriptions-item label="上报状态">
            <el-tag :type="getReportStatusType(detailData.reportStatus)" size="small">
              {{ getReportStatusText(detailData.reportStatus) }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="启用状态">
            <el-tag :type="getEnableStatusType(detailData.enableStatus)" size="small">
    ...
    <!-- 定时同步设置对话框 -->
    <el-dialog v-model="showSyncTimeDialog" title="定时同步设置" width="500px" :close-on-click-modal="false">
      <el-form :model="syncTimeForm" label-width="120px">
        <el-form-item label="模块名称">
          <el-input v-model="syncTimeForm.moduleName" disabled placeholder="信息化系统建设数据" />
        </el-form-item>
        <el-form-item label="同步时间">
          <el-date-picker v-model="syncTimeForm.syncTime" type="datetime" placeholder="选择同步时间"
            format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" />
        </el-form-item>
        <el-form-item label="启用状态">
          <el-switch v-model="syncTimeForm.enabled" active-text="启用" inactive-text="禁用" />
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="syncTimeForm.remark" type="textarea" :rows="3" placeholder="请输入备注信息" />
        </el-form-item>
      </el-form>
    ...
    <!-- 导入对话框 -->
    <el-dialog title="导入信息化系统建设数据" v-model="importVisible" width="400px" :close-on-click-modal="false">
    ...
    <!-- 更新采集日期对话框 -->
    <el-dialog title="更新采集日期" v-model="updateCollectionDateVisible" width="400px" :close-on-click-modal="false">
      <el-form :model="collectionDateForm" label-width="100px">
        <el-form-item label="采集日期" prop="collectionDate">
          <el-date-picker v-model="collectionDateForm.collectionDate" type="datetime" placeholder="请选择采集日期"
            style="width: 100%" value-format="YYYYMMDD HHmmss" format="YYYY-MM-DD HH:mm:ss" />
        </el-form-item>
        <el-form-item>
          <div class="dialog-tip">
            <el-alert title="此操作将更新选中数据的采集日期，请先选择数据后再操作！" type="warning" :closable="false" show-icon />
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="updateCollectionDateVisible = false">取消</el-button>
        <el-button type="primary" @click="submitUpdateCollectionDate" :loading="updateCollectionLoading">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  type SyncSchedule,
  type DictOption,
} from "@/api-data/data/data_base/zygxx/ods_xxhxtjssj";
...
const handleUpdateCollectionDate = () => {
  collectionDateForm.collectionDate = ""; // 重置表单
...
const submitUpdateCollectionDate = async () => {
  ...
  try {
    ...
    const ids: number[] = []; // 空数组表示更新全部数据
    const response = await batchUpdateCollectionDate(collectionDateForm.collectionDate, ids);
    const responseData = response.data as any;

    if (responseData && responseData.code === 200) {
      ElMessage.success("全部数据的采集日期更新成功");
      updateCollectionDateVisible.value = false;
      getList(); // 刷新列表
    } else {
      const msg = responseData?.message || "更新失败";
      ElMessage.error(msg);
    }
  } catch (error: any) {
    if (error !== "cancel") {
      console.error("更新采集日期失败:", error);
      ElMessage.error(error.message || "更新采集日期失败");
    }
  } finally {
