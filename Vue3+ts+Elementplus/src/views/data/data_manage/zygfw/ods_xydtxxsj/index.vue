  ...
  <div class="custom-management-container">
    ...
    <el-card class="search-card" shadow="never">
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" class="search-form">
          <el-form-item label="动态标题" prop="xydtxxbt">
            <el-input
              v-model="queryParams.xydtxxbt"
              placeholder="请输入校园动态信息标题"
              clearable
              style="width: 220px;"
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">校园动态信息数据数据清洗</div>
      ...
      <div class="table-body">
        ...
        >
          ...
          <el-table-column prop="xydtxxbt" label="动态标题" min-width="250" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="xydtxxwbfwlj" label="外部访问链接" min-width="200" show-overflow-tooltip sortable="custom">
             <template #default="{ row }">
              <el-link :href="row.xydtxxwbfwlj" type="primary" target="_blank" v-if="row.xydtxxwbfwlj">{{ row.xydtxxwbfwlj }}</el-link>
            </template>
          </el-table-column>
          <el-table-column prop="dtfbrq" label="发布日期" min-width="120" align="center" sortable="custom" />
    ...
    >
      <el-form v-if="dialogMode !== 'view'" ref="formRef" :model="form" :rules="formRules" label-width="140px">
        <el-row :gutter="20">
          ...
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="动态标题" prop="xydtxxbt">
              <el-input v-model="form.xydtxxbt" placeholder="请输入校园动态信息标题" maxlength="300" show-word-limit />
            </el-form-item>
          </el-col>
           <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="外部访问链接" prop="xydtxxwbfwlj">
              <el-input v-model="form.xydtxxwbfwlj" type="textarea" :rows="3" placeholder="请输入校园动态信息外部访问链接" maxlength="2000" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="发布日期" prop="dtfbrq">
              <el-date-picker v-model="form.dtfbrq" type="date" placeholder="请选择动态发布日期" format="YYYYMMDD" value-format="YYYYMMDD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="数据采集时间" prop="sjcjsj">
              <el-date-picker v-model="form.sjcjsj" type="datetime" placeholder="请选择数据采集时间" format="YYYY-MM-DD HH:mm:ss" value-format="YYYYMMDDHHmmss" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
      ...
      <div v-else class="detail-view">
        <el-descriptions class="detail-descriptions" :column="2" size="large" border>
          ...
          <el-descriptions-item label="动态标题" :span="2"><div class="detail-value">{{ form.xydtxxbt || "-" }}</div></el-descriptions-item>
          <el-descriptions-item label="外部访问链接" :span="2">
            <el-link :href="form.xydtxxwbfwlj" type="primary" target="_blank" v-if="form.xydtxxwbfwlj">{{ form.xydtxxwbfwlj }}</el-link>
            <span v-else>-</span>
          </el-descriptions-item>
          <el-descriptions-item label="发布日期"><div class="detail-value">{{ form.dtfbrq || "-" }}</div></el-descriptions-item>
...
import {
  ...
  updateOdsXydtxxsj,
  deleteOdsXydtxxsj,
  batchDeleteOdsXydtxxsj,
  exportOdsXydtxxsj,
  getSystemSchoolCode,
  type OdsXydtxxsjInfo,
  type OdsXydtxxsjQuery,
  type OdsXydtxxsjForm,
} from "@/api-data/data/data_base/zygfw/ods_xydtxxsj";

// 响应式数据
const loading = ref(false);
const dataList = ref<OdsXydtxxsjInfo[]>([]);
const total = ref(0);
...
const dialogTitle = computed(() => {
  switch (dialogMode.value) {
    case "add": return "新增校园动态信息数据";
    case "edit": return "编辑校园动态信息数据";
    case "view": return "查看校园动态信息数据";
    default: return "校园动态信息数据";
  }
});

// 搜索条件
const queryParams = reactive<OdsXydtxxsjQuery>({
  xydtxxbt: "",
  pageNum: 1,
  pageSize: 20,
});
...
// 表单相关
const form = reactive<OdsXydtxxsjForm>({
  id: undefined,
  zjsjwyxbs: "",
  xxdm: "",
  xydtxxbt: "",
...
const formRules = reactive({
  ...
  xydtxxbt: [{ required: true, message: "请输入校园动态信息标题", trigger: "blur" }, { max: 300, message: "长度不能超过300位", trigger: "blur" }],
  xydtxxwbfwlj: [{ required: true, message: "请输入校园动态信息外部访问链接", trigger: "blur" }, { max: 2000, message: "长度不能超过2000位", trigger: "blur" }],
  dtfbrq: [{ required: true, message: "请选择动态发布日期", trigger: "change" }],
...
const getList = async () => {
  loading.value = true;
  try {
    const response = await getOdsXydtxxsjPage(queryParams);
...
const resetForm = () => {
  Object.assign(form, {
    id: undefined, zjsjwyxbs: "", xxdm: systemSchoolCode.value, xydtxxbt: "", xydtxxwbfwlj: "",
    dtfbrq: "", sjcjsj: "", enableStatus: "1", reportStatus: undefined,
  });
...
const handleEdit = async (row: OdsXydtxxsjInfo) => {
  try {
    const response = await getOdsXydtxxsjById(row.id);
    if (response.data.code === 200) {
...
const handleView = async (row: OdsXydtxxsjInfo) => {
  try {
    const response = await getOdsXydtxxsjById(row.id);
    if (response.data.code === 200) {
      Object.assign(form, response.data.data);
...
const handleDelete = async (row: OdsXydtxxsjInfo) => {
  try {
    await ElMessageBox.confirm("确定要删除这条数据吗？", "提示", { type: "warning" });
    const response = await deleteOdsXydtxxsj(row.id);
    if (response.data.code === 200) {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const response = await batchDeleteOdsXydtxxsj(selectedIds.value);
    if (response.data.code === 200) {
...
const handleExport = async () => {
  try {
    const response = await exportOdsXydtxxsj(queryParams);
    const blob = new Blob([response.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const link = document.createElement("a");
    link.href = window.URL.createObjectURL(blob);
    link.download = `校园动态信息数据_${new Date().getTime()}.xlsx`;
    link.click();
...
const handleFormSubmit = async () => {
  ...
  try {
    ...
    const response = dialogMode.value === "add" ? await addOdsXydtxxsj(form) : await updateOdsXydtxxsj(form);
    if (response.data.code === 200) {
...
const handleSelectionChange = (selection: OdsXydtxxsjInfo[]) => { selectedIds.value = selection.map((item) => item.id); };
const handleRowDblClick = (row: OdsXydtxxsjInfo) => { handleView(row); };
