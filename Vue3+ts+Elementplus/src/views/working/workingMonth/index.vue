  ...
  <div class="app-container">
    <div class="filter-container">
      <el-card>
        <div class="flex items-center">
          ...
          <el-button type="primary" @click="loadMonthlyReportTables">
    ...
    <el-card class="mt-4">
      ...
      >
        ...
        <el-table-column label="上报月份" width="120" align="center">
...
import {
  workingMonthApi,
  type WorkingMonthVO,
} from "@/api/working/workingMonth";
import { useRouter } from "vue-router";

// 响应式数据
const loading = ref(false);
const tableData = ref<WorkingMonthVO[]>([]);
...
const monthRangeText = computed(() => {
  if (!tableData.value.length) return "本月";

  const firstRow = tableData.value[0];
  if (firstRow.year && firstRow.month) {
    return `${firstRow.year}年${String(firstRow.month).padStart(2, "0")}月`;
  }
...
function handleEdit(row: WorkingMonthVO) {
  // 调用上报接口，传递表编码和数据表英文名
  submitReport(row);
}

function handleEditData(row: WorkingMonthVO) {
  // 跳转到数据管理页面
  const path = `/data/data_manage/${row.subsetCode.toLowerCase()}/${row.tableName.toLowerCase()}`;
  router.push(path);
}

async function submitReport(row: WorkingMonthVO) {
  try {
    const res = await workingMonthApi.submitReport({
      tableId: row.tableId,
    ...
    if (res.data && res.data.code === 200) {
      ...
      // 跳转到Report页面
      router.push({
        name: "WorkingMonthReport", // 使用统一的Report页面
        query: {
...
async function loadMonthlyReportTables() {
  loading.value = true;
  try {
    const res = await workingMonthApi.getUserMonthlyReportTables(queryParams);
    console.log(res);
    if (res.data && res.data.code === 200) {
