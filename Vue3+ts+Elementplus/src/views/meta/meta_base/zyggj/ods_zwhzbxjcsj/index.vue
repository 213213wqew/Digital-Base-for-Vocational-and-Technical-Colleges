  ...
  <div class="cooperation-management">
      ...
      <div class="search-bar">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" class="search-form">
          <el-form-item label="机构/项目名称" prop="hzbxmc">
            <el-input v-model="queryParams.hzbxmc" placeholder="请输入合作办学机构/项目名称" clearable style="width: 240px;"
              @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item label="外方院校名称" prop="wfyxmc">
            <el-input v-model="queryParams.wfyxmc" placeholder="请输入外方院校名称" clearable style="width: 220px;"
              @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item label="合作国家" prop="hzgjmc">
            <el-input v-model="queryParams.hzgjmc" placeholder="请输入合作国家名称" clearable style="width: 200px;"
              @keyup.enter="handleQuery" />
          </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">中外合作办学基础数据管理</div>
      ...
      <div class="table-body">
        <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange" border
          style="width: 100%" highlight-current-row @row-click="handleRowClick" @row-dblclick="handleRowDblClick"
          :height="'calc(75vh - 160px)'" stripe size="default" :row-style="{ height: '42px' }"
          ...
          <el-table-column prop="hzbxlbText" label="合作办学类别" min-width="120" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="hzbxmc" label="合作办学机构/项目名称" min-width="250" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="wfyxmc" label="外方院校名称" min-width="200" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="hzgjmc" label="合作国家" min-width="120" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="kszymc" label="开设专业" min-width="150" show-overflow-tooltip sortable="custom" />
          <el-table-column prop="zxss" label="在校生数" min-width="100" align="center" sortable="custom" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="1000px" :close-on-click-modal="false" append-to-body
      class="cooperation-dialog" destroy-on-close @close="handleFormClose">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="合作办学类别" prop="hzbxlb">
              <el-select v-model="form.hzbxlb" placeholder="请选择合作办学类别" style="width: 100%" clearable>
                <el-option v-for="item in dictData.hzbxlbOptions" :key="item.value" :label="item.label"
                  :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="合作国家代码" prop="hzgjdm">
              <el-input v-model="form.hzgjdm" placeholder="请输入合作国家代码" maxlength="6" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="合作办学机构/项目名称" prop="hzbxmc">
              <el-input v-model="form.hzbxmc" placeholder="请输入合作办学机构/项目名称" maxlength="350" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="外方院校名称" prop="wfyxmc">
              <el-input v-model="form.wfyxmc" placeholder="请输入外方院校名称" maxlength="300" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="合作国家名称" prop="hzgjmc">
              <el-input v-model="form.hzgjmc" placeholder="请输入合作国家名称" maxlength="60" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="合作国家所属大洲" prop="hzgjssdz">
              <el-input v-model="form.hzgjssdz" placeholder="请输入合作国家所属大洲" maxlength="20" show-word-limit />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="招生开始年份" prop="zsksnf">
              <el-date-picker v-model="form.zsksnf" type="year" placeholder="请选择招生开始年份" format="YYYY"
                value-format="YYYY" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="招生截止年份" prop="zsjznf">
              <el-date-picker v-model="form.zsjznf" type="year" placeholder="请选择招生截止年份" format="YYYY"
                value-format="YYYY" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="开办期数" prop="kbqs">
              <el-input-number v-model="form.kbqs" :min="0" :max="99" controls-position="right" style="width: 100%"
                placeholder="请输入开办期数" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="每期招生人数" prop="mqzsrs">
              <el-input-number v-model="form.mqzsrs" :min="0" :max="99999999" controls-position="right"
                style="width: 100%" placeholder="请输入每期招生人数" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="在校生数" prop="zxss">
              <el-input-number v-model="form.zxss" :min="0" :max="99999999" controls-position="right"
                style="width: 100%" placeholder="请输入在校生数" />
            </el-form-item>
          </el-col>
          <el-col :span="12" style="margin-bottom: 18px;">
            <el-form-item label="开设专业号" prop="kszys">
              <div style="display: flex; align-items: center; gap: 8px">
                <el-select v-model="form.kszys" placeholder="请选择开设专业号" style="width: 100%"
                  :remote-method="getZyOptionsData" filterable clearable remote @change="handleKszysChange"
                  :loading="zyLoading">
                  <el-option v-for="option in zyOptions" :key="option.value" :label="option.label"
                    :value="option.value" />
                </el-select>
          ...
          <el-col :span="24" style="margin-bottom: 18px;">
            <el-form-item label="开设专业名称" prop="kszymc">
              <div style="display: flex; align-items: center; gap: 8px">
                <el-select v-model="form.kszymc" placeholder="请选择开设专业名称" style="width: 100%"
                  :remote-method="getZyOptionsData" filterable clearable remote @change="handleKszysChange"
                  :loading="zyLoading">
                  <el-option v-for="option in zyOptions" :key="option.value" :label="option.label"
                    :value="option.value" />
    ...
    <!-- 导入对话框 -->
    <el-dialog v-model="importDialogVisible" title="导入中外合作办学基础数据" width="400px" :close-on-click-modal="false">
      ...
      <el-upload ref="uploadRef" drag accept=".xls,.xlsx" :limit="1" :auto-upload="true" :before-upload="beforeUpload"
...
const getZyOptionsData = async (keyword?: string) => {
  const response = await getZyOptions(keyword);
...
const loadDictData = async () => {
  try {
    const [hzbxlbRes] = await Promise.all([
      getDictOptions('ZWHZBXLBDM'), // 中外合作办学类别代码
    ]);
...
1. 第一行为字段中文名称，第二行开始录入数据
2. 如果唯一标识为空，系统将自动生成
3. 采集时间将统一设置为当前时间
4. 支持.xls和.xlsx格式文件，大小不超过10MB`;
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `中外合作办学基础数据_导入模板_${new Date().getTime()}.xlsx`;
    link.click();
    window.URL.revokeObjectURL(url);
...
const customHttpRequest = async (options: any) => {
  ...
  try {
    const response = (await importCooperationData(file)) as any;
