  ...
  <div class="ods-zyszsj-management">
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">专业设置数据管理</div>
      ...
      <div class="import-content">
        <div class="import-instructions">
          ...
          >
            <template #default>
              <div>
                <p>1. 请下载并使用标准导入模板，模板包含两个工作表：数据模板和字典说明</p>
                <p>2. 支持Excel格式文件(.xlsx, .xls)，文件大小不超过10MB</p>
                <p>3. 学校代码将自动设置为当前系统学校代码，无需手动填写</p>
                <p>4. 主键数据唯一性标识可以留空，系统会自动生成</p>
                <p>5. 请参考字典说明工作表填写代码类字段，确保数据格式正确</p>
                <p>6. 日期格式说明：</p>
                <ul style="margin-left: 20px;">
                  <li>批准设置年月、首次招生年月：YYYYMM格式，如202301</li>
                  <li>数据采集时间：YYYYMMDD HHMMSS格式，如20230101 120000</li>
                </ul>
                ...
                <div v-if="importResult.errors && importResult.errors.length > 0">
                  ...
                  <ul style="margin: 5px 0; padding-left: 20px">
                    ...
                    <li
                      ...
                      style="margin: 2px 0; color: #909399"
                    >
...
const handleImportSubmit = async () => {
  ...
  if (fileType !== 'xlsx' && fileType !== 'xls') {
    ElMessage.warning("请上传Excel文件（.xlsx或.xls格式）");
    return;
  }
  ...
  try {
    // 调用导入API
    const response = await importOdsZyszsj(selectedFile.value);
    ...
    if (response.data.code === 200) {
      ...
      importResult.value = {
        ...
        successCount: 0,
        failCount: 0,
        errors: [response.data.message || "导入失败，请检查文件格式和数据"]
      };
    }
  } catch (error: any) {
    ...
    importResult.value = {
      ...
      successCount: 0,
      failCount: 0,
      errors: [error.message || "导入失败，请稍后重试"]
    };
  } finally {
...
const handleDownloadTemplate = async () => {
  ...
  try {
    ...
    const templateData = [
      {
        "主键数据唯一性标识": "", // 可以留空，系统会自动生成
        "学校代码": systemSchoolCode.value || "", // 使用系统学校代码
        "专业所属院系部名称": "信息工程学院",
        "专业名称": "计算机应用技术",
        "专业号": "510201",
        "所属专业群名称": "计算机应用专业群",
        ...
        "重点专业级别码": "01",
        "重点专业类型码": "01",
        "数据采集时间": "20231201 120000"
      }
    ];

    // 添加字典说明
    const dictSheet = [
      { "字段": "专业层次码", "说明": "51-高职(专科)" },
      { "字段": "学制码", "说明": "01-一年制, 02-二年制, 03-三年制, 04-四年制, 05-五年制" },
      { "字段": "授予学位类别码", "说明": "01-工学, 02-理学, 03-文学, 04-经济学, 05-管理学, 06-法学, 07-教育学, 08-医学, 09-农学, 10-军事学, 11-艺术学" },
      { "字段": "是否重点专业", "说明": "0-否, 1-是" },
      { "字段": "重点专业级别码", "说明": "01-国家级, 02-省级, 03-市级, 04-校级" },
      { "字段": "重点专业类型码", "说明": "01-特色专业, 02-示范专业, 03-骨干专业, 04-重点专业" }
    ];

    // 创建工作簿
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(templateData);
    const dictWs = XLSX.utils.json_to_sheet(dictSheet);
    ...
    dictWs['!cols'] = dictColWidths;

    // 添加工作表
    XLSX.utils.book_append_sheet(wb, ws, "专业设置数据模板");
    XLSX.utils.book_append_sheet(wb, dictWs, "字典说明");

    // 导出文件
    const fileName = `专业设置数据导入模板_${new Date().toISOString().slice(0, 10)}.xlsx`;
