  ...
  <div class="entity-management">
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-header">
        ...
        <div class="header-title">实习基础数据子类表</div>
    ...
    >
      <div style="margin-bottom: 20px;">
        ...
        >
          <template #default>
            <div v-html="importTipContent"></div>
          </template>
        </el-alert>
...
import {
  ...
  importInternshipData,
  downloadTemplate,
  type OdsSxjcsjInfo,
  type OdsSxjcsjQuery,
} from "@/api/meta/meta_base/zygjx/ods_sxjcsj";
...
// 导入相关方法
const importTipContent = computed(() => {
  return `
...
const handleDownloadTemplate = async () => {
  try {
    ...
    if (blob.size === 0) {
      throw new Error("下载的模板文件为空，请联系管理员");
    }
    ...
    ElMessage.error(error.message || "下载模板失败，请重试");
  }
};
...
const handleConfirmImport = async () => {
  ...
  try {
    ...
    const response = await importInternshipData(formData);
    const apiResponse = response as unknown as {
